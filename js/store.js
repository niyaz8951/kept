...
const Store = (() => {
 ...
 function merge(newRows){ // dedupe by hash, never resurrect a deleted row; returns {added, total}
  const cur=load(), seen=new Set(cur.map(r=>r[5]||hash(r))), dead=new Set(tombs());
  let added=0;
  newRows.forEach(r=>{ const h=hash(r); if(!seen.has(h)&&!dead.has(h)){ seen.add(h); cur.push([r[0],r[1],r[2],r[3]||"",r[4]||"",h]); added++; } });
 ...
 function replaceAll(rows){ clearTombs(); save(rows.map(r=>[r[0],r[1],r[2],r[3]||"",r[4]||"",hash(r)])); }
 ...
 return {hash,load,save,txns,merge,removeHash,replaceAll,clear,meta,setMeta,exportJSON,tombs,clearTombs};
})();
